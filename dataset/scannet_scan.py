...
class ScannetScan(object):
    ...
    def load_all_objects_by_maskcluster(self):
        npz = np.load(
            os.path.join('/home/xf/codes/3dvg/insSeg_res/maskcluster-seg', self.scan_id,
        ...
        print(f'{self.scan_id} object num: {len(all_objects)}')
        self.three_d_objects = all_objects

    def load_all_objects_by_pg(self):
        if self.sample:
            pc_map = {p: i for i, p in enumerate(self.selected_pis)}
        path = '/home/xf/codes/3dvg/insSeg_res/pointgroup_seg/scannetv2'
        if os.path.exists(path + '/train/' + self.scan_id + '.txt'):
            inst_list = pd.read_table(path + '/train/' + self.scan_id + '.txt', header=None)
            _dir = 'train'
        elif os.path.exists(path + '/val/' + self.scan_id + '.txt'):
            inst_list = pd.read_table(path + '/val/' + self.scan_id + '.txt', header=None)
            _dir = 'val'
        else:
            inst_list = pd.read_table(path + '/test/' + self.scan_id + '.txt', header=None)
            _dir = 'test'
        ...
        for inst_id, inst_pg in enumerate(inst_list[0]):
            ...
            inst_pred = np.loadtxt(os.path.join(path, _dir, txt_path))
            if self.sample:
                inst_pred = inst_pred[self.selected_pis]
            object_pc = np.nonzero(inst_pred)[0]
            counts = np.bincount(pc_ins_ids[object_pc])
            instance_id = np.argmax(counts)
            instance_label = id2ins[instance_id]
            all_objects.append(ThreeDObject(inst_id, object_pc, instance_label))
