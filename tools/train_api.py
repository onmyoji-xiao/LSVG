...
def make_batch_keys(model_cfg, extras=None):
    batch_keys = ['objects', 'tokens', 'target_pos','clip_feats']  # all models use these
...
def evaluate_on_dataset(model, data_loader, device, pad_idx, cfg, randomize=False, tokenizer=None, visual=False):
    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        # Forward pass
        LOSS, CLASS_LOGITS, LANG_LOGITS, LOGITS, attmap = model(batch)
        LOSS = LOSS.mean()
        res = {}
        res['logits'] = LOGITS
        ...
        if cfg.obj_cls_alpha > 0 and res['class_logits'] is not None:
